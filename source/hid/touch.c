...
static int touch_wait_event(u8 event, u8 status, u32 timeout, u8* buf) {
    ...
    while (true) {
        ...
        if (tmp[1] == event && tmp[2] == status) {
            if (buf) memcpy(buf, &tmp[3], 5);
            return 0;
        }

        if (get_tmr_ms() > timer) return 1;
    }
}
...
static void _touch_process_contact_event(touch_event* event, bool touching) {
    ...
    if (touching) {
        ...
        u16 tmp = 0x40;
        if ((event->raw[7] & 0x3F) != 1 && (event->raw[7] & 0x3F) != 0x3F) tmp = event->raw[7] & 0x3F;
        event->z /= tmp + 0x40;
...
static void _touch_parse_event(touch_event* event) {
    ...
    switch (event->type) {
        ...
        case STMFTS_EV_NO_EVENT:
            if (event->touch) event->type = STMFTS_EV_MULTI_TOUCH_MOTION;
            break;
        default:
...
touch_panel_info_t* touch_get_panel_vendor() {
    ...
    if (touch_command(STMFTS_VENDOR, &cmd, 1)) return NULL;
...
int touch_get_fw_info(touch_fw_info_t* fw) {
    ...
    if (!res) {
        ...
        cmd[2]++;
        res = touch_read_reg(cmd, 3, buf, 8);
        if (!res) fw->fw_rev = (buf[7] << 8) | buf[6];
    }
...
int touch_panel_ito_test(u8* err) {
    ...
    if (touch_sys_reset()) return res;

    // Do ITO Production test.
    u8 cmd[2] = {1, 0};
    if (touch_command(STMFTS_ITO_CHECK, cmd, 2)) return res;
...
int touch_sense_enable() {
    ...
    if (touch_command(STMFTS_SWITCH_SENSE_MODE, &cmd, 1)) return 0;
...
int touch_execute_autotune() {
    // Reset touchscreen module.
    if (touch_sys_reset()) return 0;

    // Trim low power oscillator.
    if (touch_command(STMFTS_LP_TIMER_CALIB, NULL, 0)) return 0;
    msleep(200);

    // Apply Mutual Sense Compensation tuning.
    if (touch_command(STMFTS_MS_CX_TUNING, NULL, 0)) return 0;
    ...
    if (touch_command(STMFTS_SS_CX_TUNING, NULL, 0)) return 0;
    ...
    if (touch_command(STMFTS_SAVE_CX_TUNING, NULL, 0)) return 0;
...
int touch_power_on() {
    ...
    if (btn_read_vol() == (BTN_VOL_UP | BTN_VOL_DOWN)) {
        ...
        if (touch_panel_ito_test(err))
            if (!err[0] && !err[1]) return touch_execute_autotune();
    }

    // Initialize touchscreen.
    u32 retries = 3;
    while (retries) {
        if (touch_init()) return 1;
        retries--;
    }
