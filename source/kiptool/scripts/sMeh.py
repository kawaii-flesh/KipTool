...
def getPmeh(pMehJson, pmeh_index):
    ...
    fixedValue = "const FixedValues sMeh{0}FV = {{.valuesCount = {1}, .values = {{{2}}}}};".format(pmeh_index, len(pMehJson), ', '.join(values))
    param = 'const Param sMeh{0} = {{.name = "sMeh {0}", .measure = NULL, .description = NULL, .offset = {0}, .length = 4, .defaultValue = custTable.marikoGpuVoltArray[{0}], .limitsCount = 1, .limits = {{{{.type = EFixedValues, .values = &sMeh{0}FV}}}}}};'.format(pmeh_index)
    print(fixedValue)
    print(param)
...
for i in range(0, int(pmeh_count) + 1):
    with open(pmeh_dir + 'sMeh{}.json'.format(i)) as f:
