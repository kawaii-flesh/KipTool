...
Vector_t newVec(u8 typesz, u32 preallocate) {
    if (preallocate) {
        Vector_t res = {.data = calloc(preallocate, typesz), .capacity = preallocate * typesz, .count = 0, .elemSz = typesz};

        // check .data != null;
        return res;
    } else {
        Vector_t res = {.data = NULL, .capacity = 1 * typesz, .count = 0, .elemSz = typesz};
...
Vector_t vecFromArray(void* array, u32 count, u32 typesz) {
    Vector_t res = {.data = array, .capacity = count * typesz, .count = count, .elemSz = typesz};
    return res;
}
...
void vecRem(Vector_t* vec, int idx) {
    if (vec->count <= 0 || idx >= vec->count) return;
